...
export const getProviderById = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const provider = await providerService.getProviderById(String(req.params.id));
    res.status(200).json(provider);
  } catch (err) {
    next(err);
  }
...
export const verifyProvider = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const provider = await providerService.verifyProvider(String(req.params.id));
    res.status(200).json({ 
...
export const unverifyProvider = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const provider = await providerService.unverifyProvider(String(req.params.id));
    res.status(200).json({ 
